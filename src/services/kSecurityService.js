...
export async function getPdfAnalysis(page = 1, limit) {
  const params = { page: page }
  const url = new URL(`${BASE_URL}/api/v1/pdf/applications`)
  if (limit !== undefined) {
    params.limit = limit
  }
  let response
...
export async function getWindowAnalysis(page = 1, limit) {
  const params = { page: page }
...
export const getPdfDetails = async (id) => {
  const url = `${BASE_URL}/api/v1/pdf/applications/${id}`
